...
void HLTAnalyzer::apply(const std::vector<double> & inputCharge, const std::vector<double> & inputPedestal, const std::vector<double> & inputGain, std::vector<double> & HLTOutput, double & RatioTS54, double & TimeSlew, double & Pulse, TF1 *slewFit) const
{
  ...
  //double slewpar[4] = {6.47313, -0.0310964, 0., 0.};
  ...
  //std::cout << "AveragePedestal: " << AveragePedestal << std::endl;
...
void HLTAnalyzer::SolveEquations(Double_t *TS, Double_t *par, Double_t *amp) const{
  ...
  //std::cout << "initial solution: " << A3 << ", " << A4 << ", " << A5 << std::endl;

  /*  if(A3 < 0){
    //std::cout << "A3 is negative! (1)";
    ...
    if(A3 < 0){
      //std::cout << "A3 is negative! (5)";
      A3 = 0.;
      A4 = TS[4] / TS46[0];
      A5 = 0.;
      
      if(A4 < 0){
	//std::cout << "A4 is negative! (6)";
	A3 = 0.;
	A4 = 0.;
	A5 = 0.;
      }
  ...
  //std::cout << std::endl;
  //std::cout << "final solution: " << A3 << ", " << A4 << ", " << A5 << std::endl;
